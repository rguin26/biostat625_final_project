...
```{r}
train_ind <- sample(seq_len(nrow(data_subset_nn_scaled)),
                    size = floor(0.7 * nrow(data_subset_nn_scaled)))
...
nn <- neuralnet(vital_status_Alive ~ `project_id_BEATAML1.0-COHORT`,#`project_id_BEATAML1.0-COHORT`,
                data=train_nn_temp,
                hidden=c(50, 10),
                linear.output=FALSE,
                threshold=0.01)
